...
async function runPhase10DVerification() {
  ...
  const kaveri = store.cattle.find((c: any) => c.name === 'Kaveri');
  if (kaveri) {
    ...
    await notificationService.generateAllAlerts();
    const kaveriNotifications = store.notifications.filter((n: any) => n.cattle_id === kaveri.id);
    console.log(`Kaveri notifications after calving record added:`);
    kaveriNotifications.forEach((n: any) => {
